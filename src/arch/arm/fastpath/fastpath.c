...
cap_reply_cap_ptr_new_np(cap_t *cap_ptr, word_t capReplyMaster,
                         word_t capTCBPtr)
{
    cap_ptr->words[1] = (uint32_t) ksSchedContext;
...
fastpath_call(word_t cptr, word_t msgInfo)
{
    ...
    /* dest is highest prio, so go for it */
    ...
    /* only fastpath if we will stay on the same scheduling context */
...
fastpath_reply_wait(word_t cptr, word_t msgInfo)
{
    ...
    if (unlikely(caller->tcbSchedContext)) {
        slowpath(SysReplyWait);
    }
