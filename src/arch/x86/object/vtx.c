...
static uint64_t vpid_capability = 0;
...
char vtx_mem_0[4096] ALIGN(4096);
char vtx_mem_1[4096] ALIGN(4096);
char vtx_mem_2[4096] ALIGN(4096);

static char vmxon_region[4096] ALIGN(4096);

msr_bitmaps_t msr_bitmap_region ALIGN(4096);
...
{
    vtx_memory[0] = (uint32_t)vtx_mem_0;//alloc_region(PAGE_BITS);
    vtx_memory[1] = (uint32_t)vtx_mem_1;//alloc_region(PAGE_BITS);
    vtx_memory[2] = (uint32_t)vtx_mem_2;//alloc_region(PAGE_BITS);
...
BOOT_CODE bool_t vtx_init(void)
{
    ...
    vmcs_revision = x86_rdmsr_low(MSR_VM_BASIC);
    if ((feature_control & FEATURE_CONTROL_MASK) == FEATURE_CONTROL_MASK) {
        ...
        if (init_vtx_fixed_values((vm_basic_high & BIT(55 - 32)) == BIT(55 - 32))) {
            ...
            memcpy(vmxon_region, &vmcs_revision, 4);
