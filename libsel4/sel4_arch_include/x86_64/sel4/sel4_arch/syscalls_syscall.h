...
#ifndef __LIBSEL4_SEL4_ARCH_SYSCALLS_SYSCALL_H
#define __LIBSEL4_SEL4_ARCH_SYSCALLS_SYSCALL_H
...
seL4_Yield(void)
{
    ...
    asm volatile (
            ...
            "movq   %%rbx, %%rsp    \n"
            :
            : "d" ((seL4_Word)seL4_SysYield)
            : "%rbx", "%rcx", "%rsi", "%rdi", "%r11", "memory"
            );
}
...
seL4_DebugPutChar(char c)
{
    asm volatile (
            ...
            "movq   %%rbx, %%rsp    \n"
            :
            : "d" ((seL4_Word)seL4_SysDebugPutChar),
              "D" (c)
            : "rcx", "r11", "rbx", "memory"
            );
}
...
seL4_DebugCapIdentify(seL4_CPtr cap)
{
    asm volatile (
            ...
            "movq   %%rbx, %%rsp    \n"
            : "=D" (cap)
            : "d" ((seL4_Word)seL4_SysDebugCapIdentify),
              "D" (cap)
            : "%rcx", "%rbx", "r11"
            );
...
seL4_DebugNameThread(seL4_CPtr tcb, const char *name)
{
    ...
    asm volatile (
            ...
            "movq   %%rbx, %%rsp    \n"
        :
        : "d"((seL4_Word)seL4_SysDebugNameThread), "D"(tcb)
        : "%rcx", "%rbx", "r11", "memory"
    );
}
#endif
