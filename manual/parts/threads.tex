...
values that the architecture specifications have mandated to be certain values. On X86, these bits are:
\begin{itemize}
...
seL4\_CapFault, seL4\_VMFault, seL4\_UnknownSyscall, seL4\_UserException, seL4\_DebugException, seL4\_Interrupt or seL4\_TemporalFault.
...
\texttt{Bp\_num}: The hardware ID of a target breakpoint. This ID will be
a positive integer, with values ranging from \texttt{0} to
\texttt{seL4\_NumBreakpoints}. \texttt{seL4\_NumBreakpoints} is defined for each
target platform to reflect the number of available breakpoints/watchpoints.

\texttt{Vaddr}: A virtual address which forms part of the match conditions
for the triggering of the breakpoint.

\texttt{Type} is one of: \texttt{seL4\_InstructionBreakpoint}, which specifies
...
\texttt{Size} is a positive integer indicating the span of the breakpoint.
Must be zero for \texttt{seL4\_InstructionBreakpoint}.

\texttt{Rw} is
one of \texttt{seL4\_BreakOnRead}, which means the breakpoint will only be
triggered on read-access (this is the only allowed value when configuring an
instruction breakpoint); \texttt{seL4\_BreakOnWrite} which means that the
breakpoint will only be triggered on write-access, and
\texttt{seL4\_BreakOnReadWrite} which means the breakpoint will be triggered on
any access.
...
\textbf{Value sent} & \textbf{Register set by reply} & \textbf{IPC buffer location} \\
\midrule
\reg{Breakpoint address} & (same) & \ipcbloc{IPCBuffer[0]} \\
\reg{Exception reason} & (same) & \ipcbloc{IPCBuffer[1]} \\
\reg{Breakpoint ID} & (same) & \ipcbloc{IPCBuffer[2]} \\
\bottomrule
\end{tabularx}
\caption{\label{tbl:debug_exception_result_arm}Debug exception outcome on the ARM
architecture.}
\end{table}
...
\bottomrule
\end{tabularx}
\caption{\label{tbl:debug_exception_result_ia32}Debug exception outcome on the
IA-32 architecture.}
\end{table}
\fi
...
The kernel provides support for the use of hardware single-stepping. To this end
it exposes the invocation, \texttt{seL4\_TCB\_ConfigureSingleStepping}.

\subsubsection{Arguments:}

\texttt{bp\_num}: The caller is expected to select a hardware breakpoint ID to use for setting up
the single-stepping functionality. However, not all hardware platforms require hardware breakpoint
registers to provide single-stepping functionality. If the caller's hardware platform requires the
use of hardware breakpoint registers, it will use the breakpoint register given to it in \texttt{bp\_num},
...
leave \texttt{bp\_num} unchanged. If \texttt{bp\_was\_consumed} is \texttt{true}, the caller should not
attempt to re-configure \texttt{bp\_num} for Breakpoint or Watchpoint usage until the caller no longer
needs to use single-stepping.

\texttt{num\_instructions}: The kernel supports single-stepping, and also supports skipping a certain
number of instructions before delivering the single-step fault message. \texttt{Num\_instructions}
should be set to \texttt{1} when single-stepping, or any non-zero integer value to skip that many
instructions before resuming single-stepping.

Setting \texttt{num\_instructions} to \texttt{0} \textbf{disables single stepping}. This can be
done either by setting the \texttt{num\_instructions} argument to \texttt{seL4\_TCB\_ConfigureSingleStepping}
to zero, or by replying to the Debug Exception message generated by the single-stepping feature, with
a value of zero.
